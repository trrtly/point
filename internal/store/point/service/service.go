...
func New(db *db.DB) core.UserServicePointDetailStore {
...
func (s *moneyStore) List(uid int64) (*core.UserServicePointDetail, error) {
...
func (s *moneyStore) Create(m *core.UserServicePointDetail) error {
	res := s.db.Create(m)
	if res.Error != nil {
		logrus.WithFields(
			logrus.Fields{
		...
		).Errorln("create UserServicePointDetail fail")
	}
	return res.Error
}
